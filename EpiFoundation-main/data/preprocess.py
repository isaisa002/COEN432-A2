...
class Preprocessor:
    ...
    def __call__(self, adata: AnnData, batch_key: Optional[str] = None) -> Dict:
        ...
        # Select HVG in each batch if batch_key is provided
        # step 5: subset hvg
        if self.subset_hvg:
        ...
        # step 6: binning
        if self.binning:
            logger.info("Binning data with {} bins ...".format(self.binning))
            if isinstance(self.binning, int):
                ...
                binned_rows = []
                bin_edges = []
                # layer to be preprocessed refer to unbinned data, such as X, X_pca, X_umap, etc.
                ...
                for row in tqdm(layer_data):
                    ...
                    # generate bins, represent a series of bin edges
                    # np.linspace(0, 1, n_bins - 1) is the cumulative distribution function
                    # return the quantile of the data, which is the inverse of the cumulative
                    bins = np.quantile(non_zero_row, np.linspace(0, 1, n_bins - 1))
                    # bins = np.sort(np.unique(bins))
                    # NOTE: comment this line for now, since this will make the each category
                    # has different relative meaning across datasets
                    non_zero_digits = _digitize(non_zero_row, bins, side='one')
                ...
                # Layer refer to the data type, such as X, X_pca, X_umap, etc.
                ...
                for bin_number in self.binning:
                    n_bins = bin_number
                    binned_rows = []
                    bin_edges = []
                    logger.info(f"Processing binning with {n_bins} bins")
                    for row in tqdm(layer_data):
                        if row.max() == 0:
                            logger.warning(
                                "The input data contains all zero rows. Please make sure "
                        ...
                        binned_rows.append(binned_row)
                        bin_edges.append(np.concatenate([[0], bins]))
                    result_key = self.result_binned_key
                    bin_edges_key = n_bins
                    logger.info(f"Saving binned data with {n_bins} bins, result_key: {result_key}")
                    adata.layers[result_key] = np.stack(binned_rows)
                    adata.obsm[bin_edges_key] = np.stack(bin_edges)
