...
def main():
    ...
    for i in range(start_epoch, start_epoch + EPOCHS):
        ...
        if is_master:
                logger.info(f'Epoch: {i} | MVC Loss: {running_loss["mvc"]:.4f} | Cell Type Loss: {running_loss["cell"]:.4f} | Total Loss: {running_loss["total"]:.4f} | Cell Type Accuracy: {cum_acc_cell:.2f}')
        dist.barrier()
        scheduler.step()
        ...
        if i % valid_config['freq'] == 0:
            ...
            if is_master:
                logger.info(f'MVC Loss: {running_loss["mvc"]:.4f} | Cell Type Loss: {running_loss["cell"]:.4f} | Total Loss: {running_loss["total"]:.4f} | Cell Type Accuracy: {cum_acc_cell:.2f}')
