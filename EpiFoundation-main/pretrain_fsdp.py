...
def main():
    ...
    # distibuted setting, use local_rank as device id
    torch.cuda.set_device(local_rank)
    device = torch.device("cuda", local_rank)
    
    world_size = dist.get_world_size()
    seed_all(random_seed + rank)
    # use rank to determine if it is master node, instead of local_rank
    is_master = (rank == 0)
    ...
    val_sampler = SequentialDistributedSampler(val_set, batch_size=BATCH_SIZE, world_size=world_size, rank=rank, num_replicas=world_size)
    ...
    if train_config['model']['pretrained'] is not None:
        ...
        checkpoint = torch.load(train_config['model']['pretrained'], map_location='cpu')
    ...
    for i in range(start_epoch, start_epoch + EPOCHS):
        ...
        cum_acc_cell = get_reduced(cum_acc_cell, rank, 0, world_size)
        
        cum_acc_value = 100 * cum_acc_value / index
        cum_acc_value = get_reduced(cum_acc_value, rank, 0, world_size)
        for key in running_loss:
            running_loss[key] = running_loss[key] / index
            running_loss[key] = get_reduced(running_loss[key], rank, 0, world_size)
        if is_master:
        ...
        if i % save_ckpt_freq == 0 or i == EPOCHS:
            save_ckpt_fsdp(i, steps,  model, optimizer, scheduler, scaler, running_loss["total"], task_name, ckpt_dir, rank=rank)
            if is_master:
                logger.info('Model saved at epoch: {}'.format(i))
